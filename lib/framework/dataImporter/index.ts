...
import type { PubSubPublishParam } from "../pubSubController/types"
import { PUBSUB_MESSAGES } from "../pubSubController/types/messages"
import type { DataImporterContext } from "./context"
...
export abstract class DataImporter extends PubSubController {
   ...
   */
  abstract initializeImportMachine(): StateMachine<
    DataImporterContext,
    any,
    any
  >
  ...
  async initialize() {
    ...
    this.subscribe(
      `${PUBSUB_MESSAGES.RESUME}.${this.dataSourceName}`,
    ...
    this.subscribe(
      `${PUBSUB_MESSAGES.IMPORT}.${this.dataSourceName}`,
  ...
  private async resume() {
    ...
    const importService = interpret(importMachine).onTransition(
    ...
    const persitedStateData = await dataImporterDataStore.states.get(
      this.dataSourceName
    )

    if (
      persitedStateData &&
      (!version || persitedStateData?.machineVersion === version)
    ) {
  ...
  private async import(message: string, data: PubSubPublishParam) {
    ...
    const context = data.context
    await this.resume()
    if (this.startImportEvent)
      this.importService.send(this.startImportEvent, context)
  }
  ...
  private async sendConfirmDataRequestEmailEvent(
    message: string,
    data: PubSubPublishParam
  ) {
    const context = data.context
    await this.resume()
  ...
  private async updateScheduleImport(
    message: string,
    data: PubSubPublishParam
  ) {
    const context = data.context
  ...
  async onTransition(state: AnyState): Promise<void> {
    if (!state.changed)
      return /* Only persist the state when it done transition */
    ...
    const currentDataState = await dataImporterDataStore.states.get(
      this.dataSourceName
    )
